...
cITGA6 = 4; // the channel number of alpha9-integrin staining
...
print("["+sumTextWindow+"]", "file_name" + "," + "id" + "," + "a6int_AUC" + "," + "a6int_peak_height" + "," + "z_position");
...
//for (i=0; i<1; i++) {// uncomment to run only 1 image for testing
	...
	if ( endsWith(filename, ".nd2") || endsWith(filename, ".czi") || endsWith(filename, ".tif") ) {
		...
		for (id_edge=1; id_edge<n_edges+1; id_edge++) {
			...
			print(f, "a6int_intensity");
			
			// Get raw data of the E-cadherin line scan profile
			selectImage(id); Stack.setChannel(cITGA6);
			profile = getProfile();
			...
			a6int_AUC = getAUC(profile);
			a6int_peak_height = getPeakHeight(profile);
...
//			NLSmSL_cell_2 = getNLSmSL2(profile);
			...
			print("["+sumTextWindow+"]", "\n" + outputPrefix + "," + id_edge + "," + a6int_AUC + "," + a6int_peak_height + "," + z_position);
