...
inputFolder = "/Volumes/ShaoheGtech2/SMG-epi-CRISPR-Cas9-CasRx-perturbation-assays-2019-2020/20200129-0201-Cas9Tg-LV-188-317-318-401-410-411/2020-01-31-Cas9Tg-SMG-epi-LV-zStack-2d/";
inputFolder = "/Volumes/ShaoheGtech2/SMG-epi-CRISPR-Cas9-CasRx-perturbation-assays-2019-2020/20200129-0201-Cas9Tg-LV-188-317-318-401-410-411/2020-02-01-Cas9Tg-SMG-epi-LV-zStack-3d/";
...
Dialog.addString("Channel 1 name", "Phase");
Dialog.addString("Channel 2 name", "Cas9_EGFP");
Dialog.addString("Channel 3 name", "NLSmSL");
Dialog.addString("Channel 4 name", "ITGA9");
Dialog.addChoice("Process MinIP, MaxIP, middle slice or specified slice?", newArray("Min Intensity Projection", "Max Intensity Projection", "Specify Slice Number", "Middle Slice"));
...
//	c4min = 500; c4max = 5000; idC4_8bit = to8bitMinMax( idC4, typePrefix + "-" + c4name, c4min, c4max, outputFolder, outputPrefix );
	...
	for (i=0; i<flist.length; i++) {
		...
		if ( endsWith(filename, ".nd2") || endsWith(filename, ".czi") || endsWith(filename, ".tif") ) {
			...
			if ( type == "Min Intensity Projection" ) {
				...
				processSingleSlice(idMinIP, "MinIP", outputFolder, outputPrefix);
			}
...
function getMinIP(id, tempFile) {
	...
	if ( s == 1 ) { selectImage(idf1); save(tempFile); return idf1; }
	if ( s > 1 ) { selectImage(idf1); run("Z Project...", "projection=[Min Intensity]"); idMIP = getImageID(); save(tempFile); return idMIP;}
}
