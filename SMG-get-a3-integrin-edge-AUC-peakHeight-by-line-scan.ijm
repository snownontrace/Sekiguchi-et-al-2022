...
cITGA3 = 4; // the channel number of alpha9-integrin staining
...
print("["+sumTextWindow+"]", "file_name" + "," + "edge_category" + "," + "id" + "," + "a3int_AUC" + "," + "a3int_peak_height" + "," + "NLSmSL_cell_1" + "," + "NLSmSL_cell_2" + "," + "z_position");
...
//		for (id_edge=1; id_edge<n_edges*3+1; id_edge++) {
			...
			textRecord = rawLinescanFolder + outputPrefix + "-" + id_edge + "-a3int.txt";
			f = File.open(textRecord);
			print(f, "a3int_intensity");
			
			// Get raw data of the E-cadherin line scan profile
			selectImage(id); Stack.setChannel(cITGA3);
			profile = getProfile();
			...
			a3int_AUC = getAUC(profile);
			a3int_peak_height = getPeakHeight(profile);
			...
			print("["+sumTextWindow+"]", "\n" + outputPrefix + "," + edge_category + "," + id_edge + "," + a3int_AUC + "," + a3int_peak_height + "," + NLSmSL_cell_1 + "," + NLSmSL_cell_2 + "," + z_position);
