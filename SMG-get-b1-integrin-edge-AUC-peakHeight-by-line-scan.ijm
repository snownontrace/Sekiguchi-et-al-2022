...
cITGB1 = 4; // the channel number of alpha9-integrin staining
...
print("["+sumTextWindow+"]", "file_name" + "," + "edge_category" + "," + "id" + "," + "b1int_AUC" + "," + "b1int_peak_height" + "," + "NLSmSL_cell_1" + "," + "NLSmSL_cell_2" + "," + "z_position");
...
//		for (id_edge=1; id_edge<n_edges*3+1; id_edge++) {
			...
			textRecord = rawLinescanFolder + outputPrefix + "-" + id_edge + "-b1int.txt";
			f = File.open(textRecord);
			print(f, "b1int_intensity");
			
			// Get raw data of the E-cadherin line scan profile
			selectImage(id); Stack.setChannel(cITGB1);
			profile = getProfile();
			...
			b1int_AUC = getAUC(profile);
			b1int_peak_height = getPeakHeight(profile);
			...
			print("["+sumTextWindow+"]", "\n" + outputPrefix + "," + edge_category + "," + id_edge + "," + b1int_AUC + "," + b1int_peak_height + "," + NLSmSL_cell_1 + "," + NLSmSL_cell_2 + "," + z_position);
