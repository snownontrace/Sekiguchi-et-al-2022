...
def getCountDFbyPrefix(datasetPrefix, datasetFolder=None):
    ...
    # Identify the corresponding pair of filename coding and counting files
    codingFiles = glob.glob( os.path.join(datasetFolder, datasetPrefix + '*fileNameRecord.txt') )
    countFiles = glob.glob( os.path.join(datasetFolder, datasetPrefix + '*budCount.txt') )
...
def get_annotated_countDF(datasetPrefix, datasetFolder, sgRNAs,
                          time_d=None, plate_id=None):
    '''
    Read in linescan data and annotate with meta data
    
    Inputs:
    -------
        - datasetPrefix: string that specifies the dataset
        - datasetFolder: folder that contains the counting file
        - sgRNAs: list of sgRNAs
        - time_d: how many days after culture the samples were counted
        - plate_id: string that specified the culture plate
    ...
    '''
    if time_d is None:
        time_d = int(datasetPrefix.split('-')[2][-1])
        
    if plate_id is None:
        plate_id = datasetPrefix.split('-')[1]
        
    df = getCountDFbyPrefix(datasetPrefix, datasetFolder)
    
    assert len(df) == len(sgRNAs)

    # annotate data
    sample_ids = []
    for i in range(len(df)):
        sample_ids.append('_'.join([plate_id, str(time_d), f'{i+1:02}']))
    
    df['sample_id'] = sample_ids
    df['sgRNA'] = sgRNAs
    df['time_d'] = time_d
    df['groups'] = [i + '_' + str(time_d) for i in sgRNAs]
