...
for (i=0; i<flist.length; i++) {
	...
	if ( endsWith(filename, ".tif") | endsWith(filename, ".nd2") ) {
		...
		midZ = floor(nSlices/2) + 1;
		Stack.setSlice(midZ);
		run("Enhance Contrast", "saturated=0.35");
		
		setTool("multipoint");
		run("Point Tool...", "type=Hybrid color=Magenta size=Large label counter=0");
		waitForUser("Go through z stack and mark all the buds:");
		run("Measure");
		print(f, flist[i] + "\t" + nResults);

		saveAs("Tiff", outputFolder+flist[i]);
