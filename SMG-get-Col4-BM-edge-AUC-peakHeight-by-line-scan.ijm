...
outputFolder = parentFolder + inputFolderPrefix + "-BM-line-scan-output" + File.separator;
if ( !(File.exists(outputFolder)) ) { File.makeDirectory(outputFolder); }
rawLinescanFolder = outputFolder + "raw-BM-linescan-profile-record" + File.separator;
...
cCol4 = 3; // the channel number of alpha9-integrin staining
cNLSmSL = 2; // the channel number of NLS-mSarlet-I
...
print("["+sumTextWindow+"]", "file_name" + "," + "id" + "," + "Col4_AUC" + "," + "Col4_peak_height" + "," + "z_position");
...
//for (i=0; i<1; i++) {// uncomment to run only 1 image for testing
	...
	if ( endsWith(filename, ".nd2") || endsWith(filename, ".czi") || endsWith(filename, ".tif") ) {
		...
		for (id_edge=1; id_edge<n_edges+1; id_edge++) {
			...
			waitForUser("Draw a line across the bud edge from an NLSmSL positive cell");
			
			// open a text file to store the line scan profile
			textRecord = rawLinescanFolder + outputPrefix + "-" + id_edge + "-a6int.txt";
			f = File.open(textRecord);
			print(f, "Col4_intensity");
			
			// Get raw data of the E-cadherin line scan profile
			selectImage(id); Stack.setChannel(cCol4);
			profile = getProfile();
			...
			Col4_AUC = getAUC(profile);
			Col4_peak_height = getPeakHeight(profile);
...
//			NLSmSL_cell_2 = getNLSmSL2(profile);
			...
			print("["+sumTextWindow+"]", "\n" + outputPrefix + "," + id_edge + "," + Col4_AUC + "," + Col4_peak_height + "," + z_position);
